...
def set_up_variables():
    ...
    return maxLayers, maxUnits, unitDecriment, maxDropout, dropoutDecriment, batchSize, searchMaxEpochs, predictEpochs
...
def get_data(filePath):
    ...
    scaledData = scaler.transform(data.values)

    trainSplit = scaledData[3000:, :]
    testSplit = scaledData[:3000, :]
    train_act = data.values[3000:, :]
    test_act = data.values[:3000, :]
...
def hyperPerameterTweaking():
    ...
    Returns
    -------
    (tuner.get_best_hyperparameters(num_trials = 1)[0]) : model object
        this is the best preforming model
    tuner : object
    ...
    tuner = kt.Hyperband(
        ...
        directory = f"Double_Crossover\\ml",
        project_name = "GoldenCross_detector")
...
def get_model():
    preModel = load_model("Double_Crossover\\ml\\double_crossover")
    return preModel
...
filePath = "Double_Crossover\\ml_data.csv"
maxLayers, maxUnits, unitDecriment, maxDropout, dropoutDecriment, batchSize, searchMaxEpochs, predictEpochs = set_up_variables()
